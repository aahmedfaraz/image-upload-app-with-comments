...
export default function ImageUploadAndComments() {
  ...
  const handleAddComment = async (imageId: number, imageIndex: number) => {
    setError("");
              ...
              <div key={image.id} className="border rounded p-4">
                ...
                <div className="mt-4">
                  ...
                  <div className=" flex mt-2">
                    ...
                    />
                    <button
                      onClick={() => handleAddComment(image.id, index)}
                      disabled={!newComment[image.id]?.trim() || uploading}
                      className="w-[190px] px-4 py-2 bg-green-500 text-white rounded-r disabled:opacity-50"
                    >
                      {commentLoading === index ? "Submitting..." : "Submit"}
                    </button>
                  </div>
